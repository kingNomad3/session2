...
{
	get_csbi();
	if (rows < getw_rows()) return false; // redimensionner la fen�tre avant
	csbi.dwSize.Y = rows;
...
{
	get_csbi();
	if (cols < getw_cols())	return false; // redimensionner la fen�tre avant
	csbi.dwSize.X = cols;
	csbi.srWindow.Bottom += 1; // bug the windows ???  
	return set_csbi();
}
...
bool Console::setw_size(uint16_t rows, uint16_t cols)
{
	...
	if ((cols > largestSize.X) || (rows > largestSize.Y)) return false; // rentre pas dans l'�cran...
